...
def bin_bad_rate(df_field, df_label,target_label=1):
    '''
    类别字段按照坏样本占比排序编码
    :param df_field:
    :param df_label:
    ...
    cate_dict = {v: k for k, v in rever_cate_dict.items()}  # {field0:0,field1:1,field2:2}
...
class CateCoder:
    ...
    def cate_coding(self,df_field, df_label,target_label=1):
        self.cate_dict = bin_bad_rate(df_field, df_label, target_label)
...
if __name__ == '__main__':
    from pynovice.score_card.data_binning.chi_blocks import chi_blocks
    ...
    block = chi_blocks(df_field, df_label, maxCases=5, dfree=4, cf=0.1, sample_threshold=2)
